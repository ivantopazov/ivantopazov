...
class Alcor extends CI_Controller {
    ...
    public function getRenderAlcor( $item = false ){            
       if( $item !== false ){
            ...
            $set = 0;
            if( $item['collection'] === 'Обручальные кольца' ){
                $title = 'Обручальное кольцо ' . $mett_fx_1[$item['cvetMett']];
                $set++;
            }
                  
            $str_text = mb_strtolower( $item['events'] );
            $str_find = '/' . mb_strtolower( 'Помолвка' ) . '/iU';
            if ( preg_match($str_find, $str_text)) {
                $title = 'Помолвочное кольцо ' . $mett_fx_1[$item['cvetMett']];
                $set++;
            }
            
            if( $set < 1 ){
               $title = $cat_fx_1[$item['title']] . $mett_fx_1[$item['cvetMett']]; 
            }
