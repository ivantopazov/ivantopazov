...
class Mdl_users extends CI_Model {
    ...
    public function queryData( $settings = array() ){
        ...
        //if( $this->_query[$index]['in']['item'] !== false ) $this->db->where_in( $this->_query[$index]['in']['item'], $this->_query[$index]['in']['values'] );
	...
	public function user_data( $labels = false ){		
    ...
    // Получить хеш администратора
    public function userHach (){
        return md5('abc' . md5( $this->pass ) );
    }
    
    // Удалить куки авторизации
	public function remove_all_cookie (){
		$this->mdl_helper->set_cookie( 'UID', null );
		$this->mdl_helper->set_cookie( 'HASH', null );
		$this->mdl_helper->set_cookie( 'single_key', null );
	}
