...
class Mdl_orders extends CI_Model {
    ...
    public function mod_getStatus( $item = 'getStatus', $option = array() ){        
        ...
        if( count( $this->_query[$index]['result'] ) > 0 ){            
            foreach( $this->_query[$index]['result'] as $k => $v ){
                $this->_query[$index]['result'][$k]['modules'][$item] = [
                    'statusId' => $v['status'],
                    'string' => $this->getStatusInt( $v['status'] )
                ];
            }
        }
        return true;
     ...
     // Получить информацию о владельце проекта
    public function mod_getProductList( $item = 'ProductList', $option = array() ){    
        $index = ( isset( $option['index'] ) ) ? $option['index'] : false;
        if( count( $this->_query[$index]['result'] ) > 0 ){            
            foreach( $this->_query[$index]['result'] as $k => $v ){
                $this->_query[$index]['result'][$k]['modules'][$item] = json_decode( $v['products_list'], true );
            }
        }
        return true;
    }
     ...
     // Получить информацию о владельце проекта
    public function mod_getUser( $item = 'getUser', $option = array() ){    
        $index = ( isset( $option['index'] ) ) ? $option['index'] : false;
        if( count( $this->_query[$index]['result'] ) > 0 ){            
            foreach( $this->_query[$index]['result'] as $k => $v ){
                $this->_query[$index]['result'][$k]['modules'][$item] = $this->mdl_users->queryData([
                    'return_type' => 'ARR1',
                    'where' => [
                        'method' => 'AND',
                        'set' => [[
    ...
    public function getStatusInt ( $statusId = false ){        
        $text = 'Неизвестный статус';
        if( $statusId !== false ){
            $text = 'Новый';
            if( $statusId < 1 ) $text = 'Обрабатывается';
            if( $statusId > 0 ) $text = 'Отправлен в транспортную компанию';
            if( $statusId > 1 ) $text = 'Завершён';
            if( $statusId > 2 ) $text = 'Отклонён';
        }        
        return $text;
    }
}